...
def run_statistical_summary(df_ts, df_vax, df_age, df_corr):
    ...
    print(f"    - Vaccination vs Deaths/M:   r={r_vax_death:.2f}  ({strength(r_vax_death)})")
    print(f"    - GDP/capita  vs Vaccination: r={r_gdp_vax:.2f}  ({strength(r_gdp_vax)})")
    print(f"    - Healthcare  vs Cases/M:    r={r_hci_cases:.2f}  ({strength(r_hci_cases)})")
    print()
